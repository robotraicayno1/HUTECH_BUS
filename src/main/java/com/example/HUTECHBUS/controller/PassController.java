...
public class PassController {
     ...
     * - Tính toán ngày bắt đầu: Nếu đang có thẻ cũ còn hạn -> Cộng dồn từ ngày hết
     * hạn thẻ cũ.
     * - Kích hoạt thẻ mới ngay lập tức.
     * 5. Nếu còn tiền phải trả:
     * - Tạo thông tin đơn hàng đầy đủ (bao gồm voucherId để xử lý sau khi thanh
     * toán).
    ...
    public ResponseEntity<?> buyPass(@RequestBody Map<String, Object> payload,
        ...
        if (finalPrice <= 0) {
            ...
            // Nếu user đã có thẻ đang hoạt động -> Ngày bắt đầu thẻ mới = Ngày hết hạn thẻ
            // cũ
            ...
            return ResponseEntity
                    .ok(Map.of("message", "Kích hoạt chức năng thẻ thành công bằng điểm!", "redirect", "/my-tickets"));
        }

        // TRƯỜNG HỢP 2: CẦN THANH TOÁN THÊM QUA VNPAY
        // Lưu packageId, username, pointsToUse, voucherId vào OrderInfo để
        // PaymentController xử lý khi callback thành công
        String orderInfo = "PASS:" + packageId + ":" + username + ":" + pointsToUse + ":"
                + (voucherId != null ? voucherId : "NONE");
